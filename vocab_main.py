...
# Caption and Icon
pygame.display.set_caption("Spelling Invader")
...
# Spelling Question Variables
question_text = ""  # Instruction text (e.g., "Spell the word you heard:")
correct_word = ""  # The word to be spelled
user_answer = ""
spelling_question_active = True  # always in spelling question mode
...
ielts_words = [
    "aberration", "benevolent", "candid", "deleterious", "emulate", "fortuitous",
    "gregarious", "hegemony", "immaculate", "juxtapose", "loquacious",
    "magnanimous", "nonchalant", "obstinate", "pragmatic", "quintessential",
    "rescind", "superficial", "ubiquitous", "verbose"
]
...
def generate_spelling_question():
    global question_text, correct_word, user_answer, question_start_time
    word = random.choice(ielts_words)
    correct_word = word
    question_text = "Spell the word you heard:"
    user_answer = ""
    question_start_time = pygame.time.get_ticks()
    # Pronounce the chosen word.
    speak_word(word)


# Generate the first spelling question
generate_spelling_question()
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if not waiting_for_feedback and event.type == pygame.KEYDOWN:
            if event.key == pygame.K_RETURN:
                # Check the spelling (case-insensitive)
    ...
    # Display spelling prompt and current answer if not waiting for feedback.
    if not waiting_for_feedback:
        prompt_surface = font.render(question_text, True, (255, 255, 0))
